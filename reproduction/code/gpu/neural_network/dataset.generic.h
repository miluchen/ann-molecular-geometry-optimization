...
struct DataSet {
	...
	long gbatch_size;	  // gbatch_size for gradients?
	long *GGmetas;
	long *gmetas;
	long *_gmeta;
	long *presum_gmetas;
	long gcount;			// total number of rows for gradients
	...
	void buildGGmetas(long *a, int *b, long num, int dim);
	void build_gmeta(long *a, int *b, long num, int dim);
	void printInfo(void);
	void printData(void);
