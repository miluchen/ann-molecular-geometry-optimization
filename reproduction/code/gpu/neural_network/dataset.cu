...
void DataSet::readData(std::string &_fname, char **_location) {
	...
	if (!is.is_open()) {
		std::cout << "ERROR: 'readData' can not open " << _fname << std::endl;
	}
	assert(is.is_open());
...
void DataSet::nextBatch(long _size) {
	...
	if (_size != batch_size) {
		...
		if (isGrad) {
			HANDLE_ERROR(cudaFree(d_output_grads));
			HANDLE_ERROR(cudaMalloc((void **)&d_output_grads, _size * sizeof(double)));
		}
	}
	if (pos == count) {
	...
	for (int i=0; i<net_count; i++) {
		...
		for (long j=0; j<_sizes[i]; j++) {
			...
			while (atom_perm[next] >= 0 && atom_perm[atom_perm[next]] >= 0) {
				...
				if (isGrad) {
					dataSwap(input_grads[i], next, atom_perm[next], dim_ins[i]);
				}
